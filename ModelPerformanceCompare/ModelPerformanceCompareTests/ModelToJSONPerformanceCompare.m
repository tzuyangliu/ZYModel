...
- (void)testMantlePerformace {
  ...
  [self measureBlock:^{
    for (NSInteger i = 0; i < kRepeatTimes; i++) {
      __unused id json =
          [MTLJSONAdapter JSONDictionaryFromModel:weiboStatus error:nil];
    }
  }];
}
- (void)testJSONModelPerformace {
  id json = self.json;
  JSWeiboStatus *weiboStatus =
...
- (void)testFastEasyMappingPerformace {
  ...
  [self measureBlock:^{
    for (NSInteger i = 0; i < kRepeatTimes; i++) {
      __unused id json =
          [FEMSerializer serializeObject:weibo usingMapping:mapping];
    }
  }];
}
