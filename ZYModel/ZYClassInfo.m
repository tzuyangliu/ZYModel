...
ZYEncodingGetType(const char *typeEncoding) {
  ...
  len = strlen(type);
  if (len == 0)
    return ZYEncodingTypeUnknown | qualifier;

  switch (*type) {
  case 'v':
    return ZYEncodingTypeVoid | qualifier;
  case 'B':
    return ZYEncodingTypeBool | qualifier;
  case 'c':
    return ZYEncodingTypeInt8 | qualifier;
  case 'C':
    return ZYEncodingTypeUInt8 | qualifier;
  case 's':
    return ZYEncodingTypeInt16 | qualifier;
  case 'S':
    return ZYEncodingTypeUInt16 | qualifier;
  case 'i':
    return ZYEncodingTypeInt32 | qualifier;
  case 'I':
    return ZYEncodingTypeUInt32 | qualifier;
  case 'l':
    return ZYEncodingTypeInt32 | qualifier;
  case 'L':
    return ZYEncodingTypeUInt32 | qualifier;
  case 'q':
    return ZYEncodingTypeInt64 | qualifier;
  case 'Q':
    return ZYEncodingTypeUInt64 | qualifier;
  case 'f':
    return ZYEncodingTypeFloat | qualifier;
  case 'd':
    return ZYEncodingTypeDouble | qualifier;
  case 'D':
    return ZYEncodingTypeLongDouble | qualifier;
  case '#':
    return ZYEncodingTypeClass | qualifier;
  case ':':
    return ZYEncodingTypeSEL | qualifier;
  case '*':
    return ZYEncodingTypeCString | qualifier;
  case '^':
    return ZYEncodingTypePointer | qualifier;
  case '[':
    return ZYEncodingTypeCArray | qualifier;
  case '(':
    return ZYEncodingTypeUnion | qualifier;
  case '{':
    return ZYEncodingTypeStruct | qualifier;
  case '@': {
    if (len == 2 && *(type + 1) == '?')
    ...
    if (properties) {
      ...
      for (unsigned int i = 0; i < propertyCount; i++) {
        ...
        if (info->_hasCustomContainCls) {
          info->_containCls =
              modelContainerPropertyGenericClassMap[info->_name];
        }
      }
      free(properties);
    }
...
+ (instancetype)classInfoWithClass:(Class)cls {
  ...
  ZYClassInfo *info =
      CFDictionaryGetValue(classCache, (__bridge const void *)(cls));
